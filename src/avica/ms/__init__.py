...
from avica.util import read_inputfile, latest_file, save_metafile, read_metafile
from avica.sources import check_band, identify_sources_fromtarget, choose_calib_for_snr_rating
from scipy.spatial import distance

from avica import c
# from avica.diag import get_avg_amp_phase, pl_diag, get_labels_dbscan, calcscatter_fromlabels_df
...
def ff_to_tbl_names(vis, meta, metafile, refants=None, sources=None, spws=None, target='', gaintable=None, new_tbls=False, mpi=False):
    ...
    # if not metafile : metafile = str(wd / 'meta_shortfft.avica')
...
def params_check(vis, sources, refants, spws, ff_tbls, new_meta, new_tbls, mpi):
    ...
    metafile = str(wd / 'avica.meta' / 'msmeta_snrrating.avica')
...
# ---------------------- helper functions for Identifying sources using .MS for finding calibrators and phaserefence pair (check avica.sources)
...
def identify_sources_fromtarget_ms(vis, target_source, caliblist_file=None, flux_thres=0.150, min_flux=0.025, ncalib=20, flux_df=None,
    ...
    if metafolder is None: metafolder       =   Path(vis).parent / 'avica.meta'
    metafile                                =   Path(metafolder)  / 'msmeta_sources.avica'
    msmd.open(vis)
...
def identify_sources_fromsnr_ms(vis, target_source, caliblist_file=None, snr_metafile=None, outfile='',
    ...
    if not snr_metafile: snr_metafile       =   Path(vis).parent / 'avica.meta' / 'sources.avica'
    ...
    sd = identify_sources_fromtarget_ms(vis, target_source, caliblist_file=caliblist_file,
                                ...
                                hard_selection=True)
    if not outfile: outfile = str(Path(vis).parent / 'avica.meta' / 'sources_ms_snr.avica')
...
def find_phasecenter_inms(fitsfile, vis, class_searchcoord_file, sep=0.85 ):
    ...
    """
    from avica.util import read_df_out
    from avica.fitsio import df_source_fits
    dic_phase_center = {}
